...
export default function BigMax({ bets, setBets, cdd, hS }) {
  ...
  const autoSelect = () => {
    ...
    setSelectedNumbers(shuffled.slice(0, 10)); // BigMax allows 12
  };
  ...
  const handleBetTypeClick = (game_name, bet_type) => {
    ...
    if (bet_type in betRules) {
      ...
      if (
        selectedNumbers.length < rule.min ||
        selectedNumbers.length > rule.max
      ) {
        alert(
          `For ${bet_type}, you must select between ${rule.min} and ${rule.max} numbers.`
        );
        return;
      }
    }
          ...
          <div className="price-popup">
            ...
            <div className="right">
              <div className="bet-meta">
                ...
                <div className="bet-meta-type">BigMax</div>
              </div>
              <h3>Select Price</h3>
              {price >= 10 && (
                <div className="discount">
                  <b>Congrats!</b> You got <b>10% DISCOUNT</b>
                </div>
              )}
              <div className="bet-price-selection">
                ...
                <button
                  onClick={() => price < cdd.balance && setPrice(price + 1)}
                >
                  ${price + 1}
                </button>
              </div>
