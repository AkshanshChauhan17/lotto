...
export default function BigMax({ bets, setBets, cdd, hS, tDes, tDesDef, destroy }) {
  ...
  const getTotalAvailableDiscount = () => {
    const gameBets = bets.filter(bet => bet.game_name === "Big Max" && !bet.bonus);
  ...
  const getTotalUsedDiscount = () => {
    const bonusBets = bets.filter(bet => bet.game_name === "Big Max" && bet.bonus);
  ...
  const updateDiscountAmounts = (bonusAmount, isRemoving = false) => {
    setBets(prevBets =>
      prevBets.map(bet => {
        if (bet.game_name === "Big Max" && !bet.bonus) {
          ...
          if (isRemoving) {
            ...
            const totalAvailable = prevBets
              .filter(b => b.game_name === "Big Max" && !b.bonus)
      ...
      <div className="left">
        <div className="left-top">
          ...
          <div className="left-matrix step-1">
            {range(game_matrix[2].lotto_max.clickable_numbers[0], game_matrix[2].lotto_max.clickable_numbers[1]).map((e, i) => (
              <button
            ...
            {range(game_matrix[2].lotto_max.disabled_numbers[0], game_matrix[2].lotto_max.disabled_numbers[1]).map((e, i) => (
            ...
            <div className="left-controllers-row-one step-2">
              {game_matrix[2].lotto_max.controllers.row_one.map((e, i) => (
                <button
                  ...
                  disabled={e.disabled}
                  onClick={() => handleBetTypeClick("Big Max", e.name)}
                >
                  {e.name}
                </button>
              ))}
          ...
          {bets.map((e, i) => (
            <div className="bet-ar" key={i}>
              <div className="bet">
                ...
                <button className="delete" onClick={() => { bets.some(bet => bet.bonus === true) && !e.bonus ? toast.warning("Please remove all bonus bets first.") : removeBet(i); }}>
