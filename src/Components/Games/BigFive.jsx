...
export default function BigFive({ bets, setBets, cdd, hS, tDes, tDesDef, destroy }) {
  ...
  const getTotalAvailableDiscount = () => {
    const gameBets = bets.filter(bet => bet.game_name === "Big Five" && !bet.bonus);
  ...
  const getTotalUsedDiscount = () => {
    const bonusBets = bets.filter(bet => bet.game_name === "Big Five" && bet.bonus);
  ...
  const updateDiscountAmounts = (bonusAmount, isRemoving = false) => {
    setBets(prevBets =>
      prevBets.map(bet => {
        if (bet.game_name === "Big Five" && !bet.bonus) {
          ...
          if (isRemoving) {
            ...
            const totalAvailable = prevBets
              .filter(b => b.game_name === "Big Five" && !b.bonus)
